...
5. برای قابلیت «فراموشی رمز عبور» یک بار فایل `migrations/003_password_reset.sql` را Import کن (ستون‌های `password_reset_token` و `password_reset_expires` به جدول `users` اضافه می‌شوند).
